...
const SPRING = { type: 'spring', stiffness: 420, damping: 42, mass: 0.85 };
const FADE = { duration: 0.16, ease: [0.25, 0.1, 0.25, 1] };
...
export function StackPage({ children, className = '' }) {
    const dir = useNavDirection();
    const v = STACK[dir] ?? STACK.none;
    return (_jsx(motion.div, { className: `fixed inset-0 ${className}`, initial: v.initial, animate: v.animate, exit: v.exit, transition: v.transition, style: { willChange: 'transform' }, children: children }));
}
export function TabPage({ children, className = '' }) {
    const dir = useNavDirection();
